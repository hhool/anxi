...
#define LibOb_isLeapYear(y)                \
  ((((y) % 4) == 0 && ((y) % 100) != 0) || \
   ((y) % 400) == 0)  ///< Checks a year being a leap year
...
extern const stZoneAbbreviation
    stZoneAbbreviation_Ini;  ///< Initializer for stZoneAbbreviation
extern const stZoneAbbreviation
    stZoneAbbreviation_Invalid;  ///< Invalidates all entries of
                                 ///< stZoneAbbreviation
...
size_t LibOb_strftime(
    char* destination,
    size_t destinationSize,
    const char* format,
    const struct tm* tp,
    stTimeZone* pTimeZone,
    enum enLanguage*
        pLanguage);  ///< Converts struct tm to formatted character string
const char* LibOb_strptime(
    const char* source,
    const char* format,
    struct tm* tp,
    stTimeZone* pTimeZone);  ///< Converts a time string to calendrical time
                             ///< data stored in 'struct tm'.
stTimeZone LibOb_localTimeZone(
    int8_t* pDst);  ///< Retrieves time zone and dst from local system clock
                    ///< settings

int LibOb_checkStructTm(
    struct tm* pTm,
    struct tm tmCheckConfig,
    int isDuration);  ///< Checks struct tm having valid entries.
int8_t LibOb_dst(
    const char* dstStr,
    int startsWith);  ///< Returns dst code number from a given string 'UTC',
                      ///< 'STD' or 'DST' or time zone abbreviation e.g. 'CET'.
stTimeZone LibOb_timeZone(
    const char* timeZoneStr,
    int8_t* pDst);  ///< Returns time zone struct and dst code from a given time
                    ///< zone string or abbreviation e.g. 'CET', 'UTC +01'.

/* General string related scan functions
 * -------------------------------------------------- */

int isnumber(unsigned char character);  ///< Checks a character being part of a
                                        ///< number string. See isnumber .
const char* scipSpace(
    const char* source);  ///< Scans until no more white space is found.
const char* scipDigits(
    const char* source);  ///< Scans until no more digits are found.
const char* scipNonLetters(
    const char* source,
    int* type);  ///< Scans until a digit or letter character is found
const char* scanUint(const char* source,
                     unsigned int* result,
                     int scip,
                     int allowPlus,
                     int allowLaggingDot);  ///< Converts a character string to
                                            ///< a unsigned integer.
const char* scanInt(
    const char* source,
    int* result,
    int scip,
    int requiredPlus,
    int allowLaggingDot);  ///< Converts a character string to an integer.
const char* scanExpression(
    const char* source,
    char* destination,
    size_t destinationSize,
    int scip,
    int allowNumber);  ///< Scans (and copies) the next expression.

/**
 * @file work_window_menu_store.cc
...
CMenuStoreWnd::CMenuStoreWnd() : m_pOwner(NULL), m_pShadowWnd(NULL) {}

void CMenuStoreWnd::Init(CControlUI* pOwner, POINT pt) {
  if (pOwner == NULL)
    return;
  m_pOwner = pOwner;
  m_ptPos = pt;
...
void CMenuStoreWnd::AdjustPostion() {
  CDuiRect rcWnd;
...
LPCTSTR CMenuStoreWnd::GetWindowClassName() const {
  return _T("MenuWnd");
}

void CMenuStoreWnd::OnFinalMessage(HWND /*hWnd*/) {
  delete this;
}

void CMenuStoreWnd::Notify(TNotifyUI& msg) {
...
HWND CMenuStoreWnd::Create(HWND hwndParent,
                           LPCTSTR pstrName,
                           DWORD dwStyle,
                           DWORD dwExStyle,
                           int x,
...
void CMenuStoreWnd::ShowWindow(bool bShow, bool bTakeFocus) {
  if (m_pShadowWnd != NULL)
    m_pShadowWnd->ShowWindow(bShow, false);
  CWindowWnd::ShowWindow(bShow, bTakeFocus);
}

LRESULT CMenuStoreWnd::OnCreate(UINT uMsg,
                                WPARAM wParam,
                                LPARAM lParam,
                                BOOL& bHandled) {
  m_pm.Init(m_hWnd);
  ...
  CControlUI* pRoot =
      builder.Create(_T("menu_store.xml"), (UINT)0, NULL, &m_pm);
...
LRESULT CMenuStoreWnd::OnClose(UINT uMsg,
                               WPARAM wParam,
                               LPARAM lParam,
                               BOOL& bHandled) {
  if (m_pShadowWnd != NULL)
...
LRESULT CMenuStoreWnd::OnKillFocus(UINT uMsg,
                                   WPARAM wParam,
                                   LPARAM lParam,
                                   BOOL& bHandled) {
...
LRESULT CMenuStoreWnd::OnKeyDown(UINT uMsg,
                                 WPARAM wParam,
                                 LPARAM lParam,
                                 BOOL& bHandled) {
  if (wParam == VK_ESCAPE)
...
LRESULT CMenuStoreWnd::OnSize(UINT uMsg,
                              WPARAM wParam,
                              LPARAM lParam,
                              BOOL& bHandled) {
  if (!::IsIconic(*this)) {
...
LRESULT CMenuStoreWnd::OnMove(UINT uMsg,
                              WPARAM wParam,
                              LPARAM lParam,
                              BOOL& bHandled) {
  if (m_pShadowWnd != NULL) {
...
LRESULT CMenuStoreWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam) {
  LRESULT lRes = 0;
  BOOL bHandled = TRUE;
  switch (uMsg) {
    case WM_CREATE:
